<script setup>
  const { pending, data } = useFetch('https://shoti-api.libyzxy0.repl.co/api/get-shoti?apikey=shoti-1hdv16i3pl75egkudag', {
  lazy: true, 
  server: false
});
</script>
...
<template>
  <div :class="pending ? 'justify-center items-center h-[17rem]' : 'justify-between h-[17rem]'" class="flex flex-row bg-slate-600 w-full rounded shadow mb-3">
    ...
    <div :class="pending ? 'hidden' : ''" class="flex flex-col w-[65%] h-auto text-center rounded px-2 py-2">
      ...
      <p class="text-white"><b>Username:</b> {{ pending ? 'Loading...' : data.user.username }}</p>
      <p class="text-white"><b>Nickname:</b> {{ pending ? 'Loading...' : data.user.nickname }}</p>
      <p class="text-white"><b>Duration:</b> {{ pending ? 'Loading...' : data.data.duration }}</p>
      <a class="text-blue-400 mt-2" :href="'https://www.tiktok.com/@' + data?.user?.username">View User</a>
