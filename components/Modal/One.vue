...
<template>
  <div
    :class="visible == 'f' ? 'hidden' : ''"
    class="w-full h-full fixed top-0 left-0 z-[60] overflow-x-hidden overflow-y-auto"
  >
    <div
      :class="visible == 'f' ? 'mt-0 opacity-0 hidden' : 'mt-7 opacity-100'"
      class="ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto"
    >
      <div
        class="flex flex-col border shadow-sm rounded-xl bg-slate-800 border-gray-700 shadow-slate-700/[.7]"
      >
        <div
          class="flex justify-between items-center py-3 px-4 border-b border-gray-700"
        >
          <h3 class="font-bold text-white">Here's your apikey.</h3>
          <button
            @click="visible = 'f'"
            type="button"
